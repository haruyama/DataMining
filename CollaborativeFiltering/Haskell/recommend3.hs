...
top_matches :: [(String, (Set.Set String, Double))]
            -> [(String , [(String, Double)])]
top_matches productinfos =
    top_matches_sub  productinfos productinfos []


top_matches_sub :: [(String, (Set.Set String, Double))]
                -> [(String, (Set.Set String, Double))]
                -> [(String , [(String, Double)])]
                -> [(String , [(String, Double)])]
...
tanimoto :: Ord a => ((Set.Set a), Double) -> ((Set.Set a), Double) -> Double
tanimoto us1 us2 
         | (snd us1) > (snd us2) = tanimoto us2 us1
tanimoto us1 us2 =
    let intersection_size = fromIntegral(Set.size (Set.intersection (fst us1)  (fst us2)))::Double
    in
      if intersection_size == 0
      then
         0
      else
          intersection_size  / ((snd us1) + (snd us2) - intersection_size)
...
transform :: [(String, [String])] -> [(String,(Set.Set String, Double))]
transform userinfos =
    map (\x -> (fst x, (snd x, (fromIntegral (Set.size (snd x)))::Double ) ))$ Map.toList $ transform_sub userinfos Map.empty
