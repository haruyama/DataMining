...
const unsigned int COMPACTION_RATE = 4;
...
__global__ void reduce2d(int user_ceil_length, data_type* g_idata, data_type *g_odata )
{
    ...
    sdata[tid] = 0;
    if (g_idata[i] & 0x1000) ++sdata[tid];
    if (g_idata[i] & 0x0100) ++sdata[tid];
    if (g_idata[i] & 0x0010) ++sdata[tid];
    if (g_idata[i] & 0x0001) ++sdata[tid];
    if (g_idata[i + blockDim.x] & 0x1000) ++sdata[tid];
    if (g_idata[i + blockDim.x] & 0x0100) ++sdata[tid];
    if (g_idata[i + blockDim.x] & 0x0010) ++sdata[tid];
    if (g_idata[i + blockDim.x] & 0x0001) ++sdata[tid];

	__syncthreads();

#ifdef __DEVICE_EMULATION__
    if (sdata[tid] > 0) {
        printf("reduce: %d %d %d %d %d\n", tid, i, g_idata[i], g_idata[i + blockDim.x],  sdata[tid]); 
    }
#endif
...
static int parse(char* filename, int& item_length, int& user_ceil_length, char**& item_names, data_type*& data) {
    ...
    user_ceil_length =  (int)(ceil(user_length / COMPACTION_RATE * 1.0 / REDUCE_UNIT)) * REDUCE_UNIT;
    ...
    for (int i = 0; i < item_length; ++i) {
        ...
        item_names[i] = name;
        int base = i * user_ceil_length * COMPACTION_RATE;
        for (int j = 1; j < array_length; ++j) {
            int user = json_object_get_int(json_object_array_get_idx(array_object, j)); 
            int index = (base + user) / COMPACTION_RATE;
            int subindex = (base + user) % COMPACTION_RATE;
            switch(subindex) {
                case 0:
                    data[index] |= 0x1000;
                    break;
...
void calc2d(int base_i, int user_ceil_length, data_type* data_d, data_type* similarity_d) {
    ...
    if (base_j == base_i) {
        similarity_d[base_j + user] = 0;
    } else {
        similarity_d[base_j + user] = data_d[base_i + user] & data_d[base_j + user];
    }
#ifdef __DEVICE_EMULATION__
    if (similarity_d[base_j + user]) {
        printf("similarity: %d %d\n", user, similarity_d[base_j + user]);
    }
#endif
...
int main( int argc, char *argv[] )
{
	...
	result = (result_type*)malloc(sizeof(result_type)*result_size);
    ...
    for (int i = 0; i < item_length; ++i) {
        ...
        for (int j = i + 1; j < item_length; ++j) {
            ...
            result_type sim = 0.0f;
            if (sum_s > 0) {
                sim = (sum_s * 1.0f / (reduce[i] + reduce[j] - sum_s));
            }
