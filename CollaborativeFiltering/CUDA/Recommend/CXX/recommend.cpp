...
static result_type calc2(int item_length, int user_ceil_length, int i, int j, data_type* data, result_type* similarity) {

    int base_i = user_ceil_length * i;
    int base_j = user_ceil_length * j;
    result_type reduce_i = reduce(base_i, user_ceil_length, data);
    result_type reduce_j = reduce(base_j, user_ceil_length, data);

    for (int k = 0; k < user_ceil_length; ++k) {
        if (data[base_i + k] > 0.0 && data[base_j + k] > 0.0) {
            similarity[k] = 1.0;
        } else {
            similarity[k] = 0.0;
        }
...
static int calc(int item_length, int user_ceil_length, data_type* data, result_type* result) {
    ...
    result_type* similarity = (result_type*)malloc(sizeof(result_type)*user_ceil_length);
    ...
    for(int i = 0; i < item_length; ++i) {
        ...
        for(int j = i + 1; j < item_length; ++j) {
            ...
            result_type sim = calc2(item_length, user_ceil_length, i, j, data, similarity);
            result[base_i + j] = sim;
            result[base_j + i] = sim;
        }
        result[base_i + i] = 0.0;
