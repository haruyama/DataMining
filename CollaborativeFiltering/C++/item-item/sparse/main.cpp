...
void top_matches(const ptr_map<item_type, set<int> >&  items,
    ...
    unordered_map<pair<item_type, item_type>, double, boost::hash<pair<item_type, item_type> > > memo;
    ...
    BOOST_FOREACH(item_info item1, items) {
        ...
        BOOST_FOREACH(item_info item2, items) {
            ...
            double score;
            if (item1.first > item2.first) {
                unordered_map<pair<item_type, item_type>, double, boost::hash<pair<item_type, item_type> > >::iterator itr;
                itr =
                    memo.find(make_pair(item2.first, item1.first));
                score = itr->second;
                memo.erase(itr);
            } else {
                score = similarity::tanimoto(item1.second, item2.second);
                memo[make_pair(item1.first, item2.first)] = score;
            }

            if (score > 0) {
                scores.push_back(make_pair(item2.first, score));
            }
        }
...
//    top_matches(items, item_scores, MAX_RELATED_ITEMS);
    top_matches(items, MAX_RELATED_ITEMS);
